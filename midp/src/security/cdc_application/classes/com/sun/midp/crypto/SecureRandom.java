...
class SecureRandomAdapter extends SecureRandom {
    /** CDC Secure Random reference. */
    ...
    public void nextBytes(byte[] buf, int off, int len) {
        ...
        sr.nextBytes(temp);
        System.arraycopy(temp, 0, buf, off, len);
    }
     ...
     * @param len number of bytes of seed data in buf
     */ 
    public void setSeed(byte[] buf, int off, int len) {
        byte[] temp = new byte[len];
