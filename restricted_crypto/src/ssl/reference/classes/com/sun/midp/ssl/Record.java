...
class Record {
    ...
    private long cSeq = (long) 0;
    /** Server write sequence number. */
    private long sSeq = (long) 0;
    /** Current role (either client or server). */
   ...
   static byte  getNegVersion(  ){
	...
	if (ver == 0x31) {//SSL3.1(TLS 1.0)	      
             data.generateKeys31(clientRand, serverRand, masterSecret);	      	 
	}
	else {//SSL3.0
    ...
    private boolean rdRec(boolean block) throws IOException {
        ...
        if (dataLength == 0) {
            ...
            if ((inputHeader[0] < CCS) || 
                (inputHeader[0] > APP) ||
                (inputHeader[1] > (byte) (ver >>> 4)) ||    //SSL3.1
                (inputHeader[2] > (byte) (ver & 0x0f))) {   //SSL3.1
                alert(FATAL, UNEXP_MSG);
...
class CipherSuiteData {
    ...
    /** Key block size is the closest 16-byte multiple larger than  2x(hash_size + key_material + IV_size) */
    ...
    CipherSuiteData(byte suite) throws Exception {
       ...
       if (Record.getNegVersion( ) == 0x30) { //SSL3.0
             blockSize = (digestLength + keyMaterial + ivSize) << 1;
             blockSize = ((blockSize + 15) >>> 4) << 4;
             keyBlock = new byte[blockSize];            
        ...
        sd = MessageDigest.getInstance("SHA-1");
            ...
            blockSize = (digestLength + keyMaterial + ivSize) * 2;
	 }
    }
    ...
    void generateKeys31(byte[] clientRand, byte[] serverRand, 
                      byte[] masterSecret) 
            throws GeneralSecurityException {
		
	 	chopKeysBlock31(clientRand, serverRand, masterSecret);
        keyBlock = null;
        md = null;
        sd = null;
    }	
    ...
    private void chopKeysBlock(byte[] clientRand, byte[] serverRand,
     ...
     private void chopKeysBlock31(byte[] clientRand, byte[] serverRand,
                               byte[] masterSecret) 
            throws GeneralSecurityException {
        ...
        byte temp[] = new byte[clientRand.length + serverRand.length];		

        System.arraycopy(serverRand , 0, temp, 0, serverRand.length); 
		System.arraycopy(clientRand , 0, temp, serverRand.length, clientRand.length);
	 	keyBlock = Handshake.PRF(masterSecret,"key expansion", temp, blockSize);
	
        System.arraycopy(keyBlock, 0, clientMACSecret, 0, digestLength);
        System.arraycopy(keyBlock, digestLength , serverMACSecret, 0, digestLength);
	 	 
        System.arraycopy(keyBlock, digestLength*2, clientKey, 0, clientKey.length);
	 	System.arraycopy(keyBlock, digestLength*2 +clientKey.length, serverKey,0, serverKey.length);

         //additional processing for exportable code
	 	suite = encodeDigest.getAlgorithm();
        if(suiteType == Handshake.ARCFOUR_40_MD5  ) {
		clientKey= Handshake.PRF(clientKey,"client write key", temp, clientKey.length);
		serverKey = Handshake.PRF(serverKey, "server write key", temp, serverKey.length);			
        }
...
class MAC {
    ...
    byte[] getMAC(byte type, byte[] buf, int offset, int length) {
	 ...
	 if(Record.getNegVersion( ) == 0x31) { //SSL3.1
  ...
  /**
     ...
     * @return a byte array containing the MAC
     */ 
   private byte [] getMac31(byte type, byte[] buf, int off,  int len) {

	String alg = digest.getAlgorithm(); //this value can be set as a constant when d is assigned 

       byte ver = Record.getNegVersion( );
	   
       byte[] seqArr;
	byte[] macWriteKey;
	
	//tmp=seqNum + TLSCompressed.type + TLSCompressed.version + TLSCompressed.length + TLSCompressed.fragment(len)
	seqArr = Utils.longToBytes(sequenceNumber);  
	macWriteKey = macSecret;
	...
	System.arraycopy(buf, off, temp, seqArr.length + res.length, len);
       ...
       return Handshake.hmac(alg, macWriteKey, temp);
